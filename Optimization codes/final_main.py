...
def compare(instance, report, collective_report):
    ...
    if report:
        ...
        f_loc = os.getcwd()
        directory_path = os.path.join(f_loc, 'Results')
...
# ----> Info abut the instances:
    ...
    # ----> Enter the number of drones and slots in the following two lines
    num_drones = [4]
    num_slots = [7]
    ...
    for i in range(len(num_slots)):
        # ----> Enter the instance name in the following line
        instance_ = [num_drones[i], 'SB', num_slots[i], 360]
        # ----> Enter the number of randomly generated instances in the following line
        for iter_ in range(1):
            ...
            # ----> To fix the seed uncomment the following line and enter a numeric value
            # seed1 = 4116131705355157001

            random.seed(seed1)
            print('Iteration:', (10*i) + iter_+1, '====> seed = ', seed1)
            # ----> Adjust the solution time for all the models in the following line
            run(instance_, verbose=True, seed=seed1, sol_time=15)
            sol_ = compare(instance_, report=True, collective_report=True)
            new_row = {
