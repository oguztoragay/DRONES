...
def nl_pyo(data, verbose):
    ...
    for f in families:
        for j in f:
            m.cons11.add(sum(m.s[r0, i0]*m.x[j+1, r0, i0] for r0 in slot_set for i0 in drones_set) - sum(m.c[r1, i1]*m.x[j, r1, i1] for r1 in slot_set for i1 in drones_set) <= i_times)
