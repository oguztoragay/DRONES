...
def lp_pyo(data, verbose):
    ...
    m.u1 = Var(demand_set, slot_set, drones_set, domain=NonNegativeReals, initialize=0)  # t*x
    m.u2 = Var(demand_set, slot_set, drones_set, domain=NonNegativeReals, initialize=0)  # c*x
    m.u3 = Var(demand_set, slot_set, drones_set, domain=NonNegativeReals, initialize=0)  # c_{-1}*x
    m.u4 = Var(demand_set, slot_set, drones_set, domain=NonNegativeReals, initialize=0)  # s*x
    ...
    #     for r in slot_set - {1}:
    #         for j in demand_set:
    #             m.cons10j.add(m.u3[j, r, i] <= m.c[r-1, i] + (UB * m.x[1, r, i]))
    #             m.cons10k.add(m.u3[j, r, i] >= m.c[r-1, i] - (LB * m.x[1, r, i]))
    #             m.cons10l.add(m.u3[j, r, i] <= UB * (1 - m.x[1, r, i]))
    #             m.cons10m.add(m.u3[j, r, i] >= LB * (1 - m.x[1, r, i]))
    # m.cons10j1 = ConstraintList()
    # m.cons10k1 = ConstraintList()
    # m.cons10l1 = ConstraintList()
    # m.cons10m1 = ConstraintList()
    # for i in drones_set:
    #     for r in slot_set - {1}:
    #         for j in idle:
    #             m.cons10j1.add(m.u4[j, r, i] <= m.s[r, i] + (UB * (1 - m.x[j, r, i])))
    #             m.cons10k1.add(m.u4[j, r, i] >= m.s[r, i] - (LB * (1 - m.x[j, r, i])))
    #             m.cons10l1.add(m.u4[j, r, i] <= UB * (m.x[1, r, i]))
    #             m.cons10m1.add(m.u4[j, r, i] >= LB * (m.x[1, r, i]))

    # constraint: ++++++++++++++++++++++++++++++  (11a__)
    m.cons11a = ConstraintList()
    for ind_, f in indexed_families:
        for j in f:
            m.cons11a.add(sum(m.z[j+1, r, i] for r in slot_set for i in drones_set) - sum(m.u2[j, r, i] for r in slot_set for i in drones_set) <= i_times[ind_])

    # constraint: ++++++++++++++++++++++++++++++  (12a__)
    m.cons12a = ConstraintList()
    for ind_, f in indexed_families:
        for j in f:
            m.cons12a.add(sum(m.z[j+1, r, i] for r in slot_set for i in drones_set) - sum(m.u2[j, r, i] for r in slot_set for i in drones_set) >= 0)
