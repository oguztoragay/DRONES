...
i_matrix = 5  # 5 before MAX

m = ConcreteModel(name="Parallel Machines")
m.x = Var(demand_set, slot_set, drones_set, domain=Binary)  # allocation 1
m.y = Var(demand_set, demand_set, slot_set, drones_set, domain=Binary)  # allocation 2
m.c = Var(slot_set, drones_set, domain=Integers)  # allocation 3
m.z = Var(slot_set, drones_set, domain=Binary)  # allocation 4
m.w = Var(slot_set, drones_set, domain=Integers)  # allocation 5
m.v = Var(demand_set, slot_set, drones_set, domain=NonNegativeIntegers)  # allocation 6
m.lmax = Var(initialize=0)  # We should discuss it
...
# constraint 7:-------------------------------------------------------------------------- My interpretation
...
for i in drones_set:
    m.cons8.add(m.c[1, i] == sum((t_matrix[0][j]+monitor_time_matrix[j])*m.x[j, 1, i] for j in demand_set - {9}))

# constraint 9:--------------------------------------------------------------------------
m.cons9 = ConstraintList()
for i in drones_set:
    for r in slot_set - {1}:
        m.cons9.add(m.c[r, i] == m.c[r-1, i] + sum(t_matrix[jk[0]-1, jk[1]-1] for jk in demand_set_combin) + sum(monitor_time_matrix[j-1]*m.x[j, r, i] for j in demand_set))
...
for i in drones_set:
    for jk in demand_set_combin:
        ...
        for r in slot_set - {1}:
            ...
            m.cons11.add(0.5*(m.x[j, r, i] + m.x[k, r-1, i]) - m.y[j, k, r, i] >= 0)
...
for i in drones_set:
    for r in slot_set - {1, 2}:
        ...
        m.cons17.add(m.w[r-1, i] <= m.c[r-1, i])
...
for i in drones_set:
    m.cons210.add(m.c[1, i] - Drone_Charge >= -B*(1-m.w[1, i]))
...
m.const_dummy = ConstraintList()
for j in demand_set:
    for r in slot_set:
        for i in drones_set:
            m.const_dummy.add(m.v[j, r, i] <= UB * m.x[j, r, i])
            m.const_dummy.add(m.v[j, r, i] <= m.c[r, i])
            m.const_dummy.add(m.v[j, r, i] >= m.c[r, i] - UB*(1 - m.x[j, r, i]))
