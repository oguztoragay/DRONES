...
def nl_gurobipy(data, verbose=False):
    ...
    for i in drones_set:
        for r in set(slot_set) - {1}:
            model.addConstr(
                c[r, i] == c[r-1, i] + gp.quicksum(t_matrix[k-1][j-1] * x[k, r-1, i] * x[j, r, i] for j in demand_set for k in demand_set) +
    ...
    for ind_, f in indexed_families:
        for j in f:
            model.addConstr(
                gp.quicksum(s[r, i] * x[j+1, r, i] for r in slot_set for i in drones_set) -
                gp.quicksum(c[r, i] * x[j, r, i] for r in slot_set for i in drones_set) <= i_times[ind_], name=f"c11a_{ind_}_{j}"
            )
            model.addConstr(
                gp.quicksum(s[r, i] * x[j+1, r, i] for r in slot_set for i in drones_set) -
                gp.quicksum(c[r, i] * x[j, r, i] for r in slot_set for i in drones_set) >= 0, name=f"c11b_{ind_}_{j}"
            )
